...
const PrintInvoice = forwardRef(function PrintInvoice({ invoice }, ref) {
  ...
  const displayPhone = () => {
    ...
    const alt = invoice.alternatePhone && invoice.alternatePhone !== '-' 
      ? invoice.alternatePhone 
      : (invoice.alternatePhoneNo && invoice.alternatePhoneNo !== '-' ? invoice.alternatePhoneNo : '');
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', borderBottom: '2px solid #cbd5e1', paddingBottom: '10px' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div>
            <Logo />
            <div style={{ fontSize: '12px', color: '#475569', marginTop: '4px' }}>Vehicle Finance, Refinance, Sell and Buy Services</div>
            <div style={{ fontSize: '12px', color: '#475569' }}>Opp Hero Showroom, Shanthi Theater Road, Chirala, Andhra Pradesh - 523155</div>
            <div style={{ fontSize: '12px', color: '#475569', marginTop: '2px' }}>Ph: 7799772786, 9399922786</div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 0.7fr', gap: '12px', marginTop: '12px' }}>
        <div style={{ border: '1px solid #d1d5db', borderRadius: '8px', padding: '10px 12px' }}>
          <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>BILLED TO</div>
          <div style={{ fontSize: '13px', lineHeight: 1.45 }}>
            ...
            <div><strong>Phone:</strong> {displayPhone()}</div>
            <div><strong>Aadhaar:</strong> {invoice.aadhaar || '-'}</div>
            <div style={{ display: 'flex', gap: '4px', alignItems: 'flex-start' }}>
              <strong>Address:</strong>
              <span style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', flex: 1 }}>{invoice.address || '-'}</span>
            </div>
        ...
        <div style={{ border: '1px solid #d1d5db', borderRadius: '8px', padding: '10px 12px' }}>
          <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>VEHICLE DETAILS</div>
          <div style={{ fontSize: '13px', lineHeight: 1.45 }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '12px' }}>
        <thead>
          <tr style={{ background: '#0f172a', color: '#ffffff' }}>
            <th style={{ textAlign: 'left', padding: '8px 10px', fontSize: '11.5px' }}>DESCRIPTION</th>
            <th style={{ textAlign: 'left', padding: '8px 10px', fontSize: '11.5px' }}>VALUE</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td style={{ border: '1px solid #cbd5e1', padding: '7px 10px', fontSize: '12px' }}>Sale Amount</td>
            <td style={{ border: '1px solid #cbd5e1', padding: '7px 10px', fontSize: '12px' }}>Rs. {formatMoney(invoice.saleAmount)}</td>
          </tr>
          {hasFinance && (
            <>
              <tr>
              ...
              {invoice.mode === 'refinance' && invoice.oldHaNumber && invoice.oldHaNumber !== '-' && (
                <tr>
                  <td style={{ border: '1px solid #cbd5e1', padding: '7px 10px', fontSize: '12px' }}>Previous HA / Agreement No</td>
                  <td style={{ border: '1px solid #cbd5e1', padding: '7px 10px', fontSize: '12px' }}>{invoice.oldHaNumber}</td>
                </tr>
              )}
            </>
          )}
          {invoice.mode === 'buyer' && invoice.pendingAmount && invoice.pendingAmount !== '-' && (
            <>
              <tr>
                <td style={{ border: '1px solid #cbd5e1', padding: '7px 10px', fontSize: '12px' }}>Pending Amount</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '7px 10px', fontSize: '12px' }}>Rs. {formatMoney(invoice.pendingAmount)}</td>
              </tr>
              <tr>
      ...
      {invoice.mode === 'buyer' && (
        <div style={{ marginTop: '12px', border: '1px solid #d1d5db', borderRadius: '8px', padding: '8px 12px', background: '#f8fafc' }}>
          <div style={{ fontSize: '11.5px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>BUYER AGREEMENT DESCRIPTION</div>
          <p style={{ margin: 0, fontSize: '11.5px', lineHeight: 1.4, color: '#0f172a' }}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '11.5px', lineHeight: 1.4, color: '#0f172a' }}>
          ...
          <p style={{ margin: '8px 0 0', fontSize: '11.5px', lineHeight: 1.4, color: '#0f172a' }}>
      ...
      {invoice.mode === 'refinance' && (
        <div style={{ marginTop: '12px', border: '1px solid #d1d5db', borderRadius: '8px', padding: '8px 12px', background: '#f8fafc' }}>
          <div style={{ fontSize: '11.5px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>REFINANCE AGREEMENT DESCRIPTION</div>
          <p style={{ margin: 0, fontSize: '11.5px', lineHeight: 1.4, color: '#0f172a' }}>
      ...
      {invoice.mode === 'seller' && (
        <div style={{ marginTop: '12px', border: '1px solid #d1d5db', borderRadius: '8px', padding: '8px 12px', background: '#f8fafc' }}>
          <div style={{ fontSize: '11.5px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>SELLER AGREEMENT DESCRIPTION</div>
          <p style={{ margin: 0, fontSize: '11.5px', lineHeight: 1.4, color: '#0f172a' }}>
      ...
      <div style={{ marginTop: '16px', paddingTop: '10px', borderTop: '1px solid #cbd5e1', display: 'flex', justifyContent: 'space-between', gap: '24px' }}>
        <div style={{ fontSize: '12px', color: '#475569' }}>
          <div>Thank you for choosing SGB Finance.</div>
          <div>This is a system-generated invoice.</div>
          <div style={{ marginTop: '4px', fontWeight: 700, color: '#b91c1c' }}>
            Note: Rs. 300/- charges should be paid as office fee.
          </div>
        </div>
        <div style={{ display: 'flex', gap: '36px', fontSize: '12px', color: '#334155', alignItems: 'flex-end' }}>
