...
function PendingDownpayment() {
  ...
  return (
    <div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 bg-white">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-semibold">Pending Downpayment</h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="h-9 sm:h-10 px-3 sm:px-4 rounded-full bg-gray-100 border border-gray-200 flex items-center gap-2 text-xs sm:text-sm font-semibold hover:bg-gray-200 transition-colors w-full sm:w-auto justify-center sm:justify-start"
          >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="h-9 sm:h-10 w-full sm:w-64 md:w-80 rounded-full bg-gray-100 border border-gray-200 pl-9 pr-3 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-lime-400"
            />
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-3 sm:px-4 py-2 sm:py-3 text-left text-[11px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wider">
                  Name
                </th>
                  <th className="px-3 sm:px-4 py-2 sm:py-3 text-left text-[11px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wider">
                  Phone Number
            ...
            <tbody className="divide-y divide-gray-200">
              {pendingPayments.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-3 sm:px-4 py-6 sm:py-8 text-center text-xs sm:text-sm text-gray-500">
                ...
                pendingPayments.map((payment) => {
                  ...
                  return (
                    <tr key={payment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-gray-700 max-w-xs">
                        {payment.address}
                      </td>
                        ...
                        {editingAmount === payment.id ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              className="h-7 w-20 px-2 border rounded text-xs focus:outline-none focus:ring-2 focus:ring-lime-400"
                            />
                            <button
                              onClick={() => handleAmountUpdate(payment.id)}
                              className="h-7 px-2 rounded bg-lime-400 text-gray-800 text-xs font-semibold hover:bg-lime-500 whitespace-nowrap"
                            >
                              Save
                            </button>
                            <button
                              ...
                              className="h-7 px-1.5 rounded border border-gray-300 text-gray-700 text-xs font-semibold hover:bg-gray-50 whitespace-nowrap"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="h-6 w-6 rounded bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center flex-shrink-0"
                              title="Edit amount"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                        ...
                        {editingCommitment === payment.id ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              className="h-7 px-2 border rounded text-xs focus:outline-none focus:ring-2 focus:ring-lime-400 flex-1"
                            />
                            <button
                      ...
                      <td className="px-4 py-3 text-sm">
                        <select
                          ...
                          value="Pending"
                          className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs font-semibold border-2 focus:outline-none focus:ring-2 focus:ring-lime-400 ${getStatusClass(daysRemaining)}`}
                        >
      ...
      <div className="md:hidden grid grid-cols-1 gap-4">
        {pendingPayments.length === 0 ? (
          ...
          pendingPayments.map((payment) => {
            ...
            return (
              <div key={payment.id} className="bg-white border rounded-lg sm:rounded-xl shadow-sm p-3 sm:p-4 space-y-2 sm:space-y-3">
                <div className="flex items-start justify-between gap-2 sm:gap-3">
                  <div>
                    <div className="text-sm sm:text-base font-semibold text-gray-900">{payment.name}</div>
                    <div className="text-xs sm:text-sm text-gray-600">{payment.phoneNumber}</div>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700">
                  <div className="bg-gray-50 px-2 sm:px-3 py-2 rounded-lg flex-1 min-w-[120px]">
                    <div className="text-[10px] sm:text-xs text-gray-500 mb-1">Amount</div>
                    {editingAmount === payment.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-gray-900">₹{inr(payment.downpaymentAmount)}</span>
                                                <span className="text-xs sm:text-sm font-semibold text-gray-900">₹{inr(payment.downpaymentAmount)}</span>
                        <button
                          onClick={() => {
                            setEditingAmount(payment.id);
                            setAmountValue(String(payment.downpaymentAmount || ""));
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg flex-1 min-w-[140px]">
                    <span className="text-[10px] sm:text-xs text-gray-500 whitespace-nowrap">Due</span>
                    <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatDisplayDate(payment.paymentDueDate)}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <div className="text-[10px] sm:text-xs text-gray-500">Commitment date</div>
                  {editingCommitment === payment.id ? (
                    <div className="flex flex-col gap-2">
                      <input
                        ...
                        value={commitmentDate}
                        onChange={(e) => setCommitmentDate(e.target.value)}
                        className="h-8 sm:h-9 px-2 sm:px-3 border rounded text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-lime-400 w-full"
                      />
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleCommitmentUpdate(payment.id)}
                          className="flex-1 h-8 sm:h-9 px-2 sm:px-3 rounded bg-lime-400 text-gray-800 text-xs sm:text-sm font-semibold hover:bg-lime-500 whitespace-nowrap"
                        >
                          Save
                        </button>
                        <button
                          ...
                          className="flex-1 h-8 sm:h-9 px-2 sm:px-3 rounded border border-gray-300 text-gray-700 text-xs sm:text-sm font-semibold hover:bg-gray-50 whitespace-nowrap"
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs sm:text-sm text-gray-800">{formatDisplayDate(payment.commitmentDate)}</span>
                      <button
                        onClick={() => {
                          setEditingCommitment(payment.id);
                          setCommitmentDate(payment.commitmentDate);
                ...
                <div className="space-y-1">
                  ...
                  <select
                    onChange={(e) => handleStatusChange(payment.id, e.target.value)}
                    value="Pending"
                    className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-semibold border-2 focus:outline-none focus:ring-2 focus:ring-lime-400 ${statusClass}`}
                  >
                    <option value="Pending">
                      {getStatusLabel(daysRemaining)}
                    </option>
      ...
      <div className="mt-5 flex items-center justify-end gap-3">
        <button
          ...
          className="px-2 sm:px-3 py-1 rounded-full bg-gray-100 text-xs disabled:opacity-50"
        >
          previous
        </button>
      ...
      <div className="mt-5 sm:mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-white rounded-xl border shadow-sm p-3 sm:p-4">
          <div className="text-xs sm:text-sm text-gray-500 mb-1">Total Pending Amount</div>
          <div className="text-lg sm:text-2xl font-bold text-gray-900">
            ₹{inr(pendingPayments.reduce((sum, p) => sum + p.downpaymentAmount, 0))}
          </div>
        </div>
        <div className="bg-white rounded-xl border shadow-sm p-3 sm:p-4">
          <div className="text-xs sm:text-sm text-gray-500 mb-1">Total Customers</div>
          <div className="text-lg sm:text-2xl font-bold text-gray-900">
            {pagination?.totalRecords || pendingPayments.length}
          </div>
        </div>
        <div className="bg-white rounded-xl border shadow-sm p-3 sm:p-4">
          <div className="text-xs sm:text-sm text-gray-500 mb-1">Overdue Payments</div>
  ...
  <div className="md:hidden grid grid-cols-1 gap-3 sm:gap-4">
        {pendingPayments.length === 0 ? (
          <div className="bg-white border rounded-lg shadow-sm p-3 sm:p-4 text-center text-xs sm:text-sm text-gray-500">
            No pending payments found
          </div>
        ) : (
          pendingPayments.map((payment) => {
            ...
            return (
              <div key={payment.id} className="bg-white border rounded-lg sm:rounded-xl shadow-sm p-3 sm:p-4 space-y-2 sm:space-y-3">
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 px-2 sm:px-3 py-2 rounded-lg flex-1 min-w-[120px]">
                    <span className="text-[10px] sm:text-xs text-gray-500 whitespace-nowrap">Due</span>
                    <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatDisplayDate(payment.paymentDueDate)}</span>
                  </div>
                </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <div className="relative bg-white w-full sm:w-[90%] max-w-md rounded-lg sm:rounded-2xl border shadow-xl p-4 sm:p-6 mx-3 sm:mx-0">
            <div className="text-center">
              ...
              <div className="flex gap-2 sm:gap-3">
                <button
                  onClick={handleCancelPaid}
                  className="flex-1 h-9 sm:h-10 rounded-lg border-2 border-gray-300 text-gray-700 text-xs sm:text-sm font-semibold hover:bg-gray-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleConfirmPaid}
                  className="flex-1 h-9 sm:h-10 rounded-lg bg-gradient-to-b from-[#B0FF1C] to-[#40FF00] text-gray-800 text-xs sm:text-sm font-semibold hover:shadow-lg transition-shadow"
                >
                  OK
                </button>
              </div>
