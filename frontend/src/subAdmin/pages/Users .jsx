    ...
    <div className="p-6">
      <div className="md:flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 border font-semibold rounded-lg shadow hover:shadow-md transition-shadow bg-white">
        ...
        <div className="w-full max-w-sm py-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2 rounded-3xl border border-transparent bg-white/90 text-sm focus:outline-none focus:ring-2 focus:ring-[#bff86a] shadow-[0px_4px_10px_-3px_rgba(0,_0,_0,_0.7)]"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow p-4 md:p-6 overflow-hidden">
          ...
          <table className="w-full table-auto border-collapse">
            <thead>
              <tr className="bg-gray-100 text-left rounded-lg">
                <th className="py-4 px-4 text-sm font-semibold">s no</th>
                <th className="py-4 px-4 text-sm font-semibold">seller name</th>
                <th className="py-4 px-4 text-sm font-semibold">buyer name</th>
                <th className="py-4 px-4 text-sm font-semibold">vehicle no</th>
                <th className="py-4 px-4 text-sm font-semibold">sold amount</th>
                <th className="py-4 px-4 text-sm font-semibold">buy amount</th>
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id} className="border-b last:border-b-0">
                  <td className="py-4 px-4 text-sm">{u.id}</td>
                  <td className="py-4 px-4 text-sm">{u.seller}</td>
                  <td className="py-4 px-4 text-sm">{u.buyerName || '-'}</td>
                  <td className="py-4 px-4 text-sm">{u.vehicle}</td>
                  <td className="py-4 px-4 text-sm">{u.soldAmount}</td>
                  <td className="py-4 px-4 text-sm">{u.buyAmount || '-'}</td>
                  <td className="py-4 px-4 text-sm">{u.date}</td>
                  <td className="py-4 px-4">
                    {u.buyerName ? (
                      ...
                      <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-100 text-yellow-800 text-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none"><path d="M12 8v4m0 4h.01" stroke="#92400e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        <span className="capitalize">buyer pending</span>
                      </span>
                    )}
                  </td>
                  <td className="py-4 px-4">
          ...
          {users.map(u => (
            <div key={u.id} className="bg-white rounded-lg shadow p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="text-sm font-semibold">{u.id}. {u.seller}</div>
                </div>
                <div className="flex items-center gap-2">
                  {u.buyerName ? (
                    <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-100 text-green-800 text-xs">
                    ...
                    <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-100 text-yellow-800 text-xs">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" viewBox="0 0 24 24" fill="none"><path d="M12 8v4m0 4h.01" stroke="#92400e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      <span className="capitalize">buyer pending</span>
                    </span>
                  )}
                </div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-gray-700">
                <div><span className="text-xs text-gray-400">Buyer</span><div className="font-medium">{u.buyerName || '-'}</div></div>
                <div><span className="text-xs text-gray-400">Vehicle</span><div className="font-medium">{u.vehicle}</div></div>
                <div><span className="text-xs text-gray-400">Sold Amount</span><div className="font-medium">{u.soldAmount}</div></div>
                <div><span className="text-xs text-gray-400">Buy Amount</span><div className="font-medium">{u.buyAmount || '-'}</div></div>
                <div className="col-span-2"><span className="text-xs text-gray-400">Date</span><div className="font-medium">{u.date}</div></div>
              </div>

              <div className="mt-3 flex items-center justify-end gap-2">
                
                <button className="p-2   text-gray-700" title="view"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 32 32"><circle cx="16" cy="16" r="4" fill="#a6a6a6"/><path fill="#a6a6a6" d="M30.94 15.66A16.69 16.69 0 0 0 16 5A16.69 16.69 0 0 0 1.06 15.66a1 1 0 0 0 0 .68A16.69 16.69 0 0 0 16 27a16.69 16.69 0 0 0 14.94-10.66a1 1 0 0 0 0-.68M16 22.5a6.5 6.5 0 1 1 6.5-6.5a6.51 6.51 0 0 1-6.5 6.5"/></svg></button>
        ...
        <div className="mt-4 flex justify-end items-center gap-3">
          <button className="px-3 py-1 rounded-full bg-gray-100 text-sm">previous</button>
          <div className="text-sm">1</div>
          <button className="px-3 py-1 rounded-full bg-gray-100 text-sm">next</button>
        </div>
      </div>
    </div>
  )
